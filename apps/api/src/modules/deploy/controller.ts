...
import { provisionCandidate, deleteCandidate } from "../provisioning/candidate";
import {
  reserveDeploymentHeadroom,
  releaseDeploymentHeadroom,
} from "../quota/headroom";
import { AuthClient } from "./invariant";
import { redeemApprovalToken } from "./approvals-stub";
import { enqueue, queuePosition, admitNext, complete } from "./queue";
...
function view(row: DeploymentRecord): DeploymentView {
  const { userId: _u, namespace: _n, liveDeployment: _d, liveService: _s, ...rest } =
    row;
  return rest;
}
...
export async function restoreServer(
  ...
  if (confirmDataLoss !== `restore ${serverId} and discard play since snapshot`) {
    throw new Error(
      "Snapshot restore requires confirmDataLoss to name the data loss explicitly"
    );
  }
