  ...
  .ws("/api/servers/:serverId/logs", {
    async open(ws: any) {
      ...
      try {
        ...
        let namespace = k8sRecord.namespace;
        let podName = "";
        let containerName = "";
        try {
          const resolvedPod = await resolveServerLogPod(
            k8sApi,
            appsApi,
            serverId,
            k8sRecord
          ...
          if (err instanceof LogPodResolutionError) {
            const messageByCode: Record<string, string> = {
              "missing-metadata":
                "Error: Kubernetes metadata not found for this server",
              "pod-not-ready":
                "Error: Server pod is not ready yet. Please try again shortly.",
              "deployment-missing":
                "Error: Kubernetes deployment is missing for this server. Provisioning may have failed.",
              "container-missing":
                "Error: No containers found in the server pod",
            };

            ws.send(
              messageByCode[err.code] ?? "Error: Server logs unavailable"
            );
          ...
          ws.send(
            "Error: Failed to reach server resources. Please try again later."
          );
        ...
        const req = await startPodLogPolling(
          ...
          (chunk) => {
            try {
              ...
              console.error(
                `[${serverId}] Error sending log chunk:`,
                sendError
              );
            }
          }
          ...
          console.error(`[${serverId}] Failed to start log polling:`, {
            ...
            statusCode:
              typeof streamError === "object" && streamError !== null
                ? ((streamError as { code?: number; statusCode?: number })
                    .code ??
                  (streamError as { code?: number; statusCode?: number })
                    .statusCode)
                : undefined,
          ...
          try {
            ws.send(
              "[System Error] Failed to start log stream. Please try again later."
            );
        ...
        console.error(
          `[${serverId}] WebSocket log streaming error during open:`,
          err
        );
        try {
          ws.send(
