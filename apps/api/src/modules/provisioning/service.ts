...
const TEST_WORKLOAD_REPLICAS =
  TEST_EXISTING_PVC_NAME && process.env.FARLANDS_TEST_START_POD !== "true"
    ? 0
    : 1;
...
const VELOCITY_SECRET_KEY =
  process.env.FARLANDS_VELOCITY_SECRET_KEY ?? "forwarding.secret";
          ...
          containers: [
            ...
            },
            {
              ...
              command: [
                "sh",
                "-c",
                "apk add --no-cache tar >/dev/null && python3 /sync/sender.py",
              ],
              ports: [
                {
                  name: "world-sync",
    ...
    const [updatedServer] = await tx
      ...
      .where(
        and(
          ...
          eq(gameServers.currentState, "provisioning")
        )
      )
...
async function getProvisionConfig(serverId: string): Promise<ProvisionConfig> {
  ...
  if (!row) {
    throw new Error(
      `No server_configs/game_servers row found for server ${serverId}`
    );
  }
...
export async function deleteGameServer(serverId: string): Promise<boolean> {
  try {
    ...
    if (!k8sRecord) {
      console.warn(
        `[${serverId}] No Kubernetes metadata found; skipping cluster teardown`
      );
      return true;
    }
