  ...
  async fetchMojangJavaRequirement(
    ...
    try {
      ...
      const resolvedVersion =
        requestedVersion.toLowerCase() === "latest"
          ? manifest.latest?.release
          : requestedVersion;
      const versionMeta = manifest.versions?.find(
        (candidate) => candidate.id === resolvedVersion
      );
      ...
      if (error.name === "AbortError") {
        console.warn(
          `[Mojang API] Request timed out after ${timeoutMs}ms for version ${version}.`
        );
      } else {
        console.warn(
