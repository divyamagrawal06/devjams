...
import { gameServers, serverK8s, serverConfigs } from "@repo/db/schema";
import { db } from "../../db";
import {
...
import { MinecraftUtils, calculateContainerMemory } from "./utils";

const SERVER_PORT = 25565;

function candidateNames(liveServerId: string, deploymentId: string) {
  const short = deploymentId.replace(/[^a-z0-9]/gi, "").slice(0, 10).toLowerCase();
  return {
            ...
            containers: [
              {
                ...
                command: [
                  "sh",
                  "-c",
                  "echo 'candidate held; Paper not started'; sleep infinity",
                ],
